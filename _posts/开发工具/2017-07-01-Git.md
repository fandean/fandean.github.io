...
git reset有多个模式，其中`--herd`是模式之一，另还有默认的`--mixed`(不改变当前工作树)；详见：`git help reset`
...
```shell
# 添加了远程仓库后的 第一次push 添加 -u 选项
# 将当前分支内容推送给远程仓库origin的master分支。
# -u 将origin仓库的master分支设置为本地仓库当前分支的upstream(上游)。
# 如果当前分支与多个主机存在追踪关系，则可以使用-u选项指定一个默认主机，这样后面就可以不加任何参数直接使用`git push`。
